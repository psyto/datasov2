...
class IdentityService extends events_1.EventEmitter {
    constructor(config) {
        ...
        this.config = config;
        this.logger = new Logger_1.Logger("IdentityService");
        ...
        const arweaveConfig = {
            ...
            wallet: config.wallet || {}, // Wallet should be provided in config
        };
        this.storageService = new ArweaveService_1.ArweaveService(arweaveConfig);
        ...
        this.storageService.on("documentUploaded", (data) => {
            this.emit("storageEvent", {
    ...
    async connect() {
        try {
            ...
            throw new types_1.StorageError("Connection failed", {
    ...
    async getIdentity(identityId) {
        try {
            ...
            const document = await this.storageService.getLatestIdentityDocument(identityId);
            if (!document) {
                return null;
            }
            ...
            throw new types_1.StorageError("Failed to get identity", {
                identityId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    ...
    async getIdentitiesByOwner(owner) {
        try {
            this.validateConnection();
            const documents = await this.storageService.queryDocumentsByOwner(owner);
            return documents
            ...
            throw new types_1.StorageError("Failed to get identities by owner", {
                owner,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    ...
    async generateIdentityProof(identityId) {
        try {
            ...
            if (!identity) {
                throw new types_1.StorageError("Identity not found", { identityId });
            }
            if (identity.status !== types_1.IdentityStatus.VERIFIED) {
                throw new types_1.StorageError("Identity not verified", {
                    identityId,
            ...
            throw new types_1.StorageError("Failed to generate identity proof", {
                identityId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    ...
    async generateAccessProof(identityId, consumer, dataType) {
        try {
            ...
            if (!hasAccess) {
                throw new types_1.StorageError("Access not granted", {
                    identityId,
                    consumer,
                    dataType,
                });
            }
            const permission = identity.accessPermissions.find((p) => p.consumer === consumer);
            ...
            const proof = {
                identityId,
                consumer,
                permissionType: permission?.permissionType || types_1.PermissionType.READ_ONLY,
                dataTypes: [dataType],
            ...
            throw new types_1.StorageError("Failed to generate access proof", {
                identityId,
                consumer,
                dataType,
                error: error instanceof Error ? error.message : String(error),
    ...
    async validateIdentityProof(proof) {
        try {
            ...
            if (proof.storageTransactionHash) {
                try {
                    await this.storageService.getDocumentByTxId(proof.storageTransactionHash);
                }
                catch (error) {
                    return {
                        isValid: false,
    ...
    async validateAccessProof(proof) {
        try {
            ...
            const permission = identity.accessPermissions.find((p) => p.consumer === proof.consumer);
            ...
            const hasDataType = proof.dataTypes.some((dt) => permission.dataTypes.includes(dt));
            if (!hasDataType) {
                return {
                    isValid: false,
     ...
     */
    mapDocumentToDigitalIdentity(document) {
        return {
            ...
            status: document.status || types_1.IdentityStatus.PENDING,
            verificationLevel: document.kycVerification?.verificationLevel ||
                types_1.VerificationLevel.NONE,
                ...
                ? [
                    {
                        ...
                        permissionType: document.accessPermission.permissionType ||
                            types_1.PermissionType.READ_ONLY,
            ...
            revokedAt: document.status === types_1.IdentityStatus.REVOKED ? Date.now() : undefined,
    ...
    calculateValidUntil(verificationLevel) {
        ...
        switch (verificationLevel) {
            case types_1.VerificationLevel.CREDENTIAL:
                return now + 365 * oneDay; // 1 year
            case types_1.VerificationLevel.HIGH:
                return now + 180 * oneDay; // 6 months
            case types_1.VerificationLevel.ENHANCED:
                return now + 90 * oneDay; // 3 months
            case types_1.VerificationLevel.BASIC:
    ...
    validateConnection() {
        if (!this.isConnected) {
            throw new types_1.StorageError("Not connected to storage backend");
        }
    }
    /**
